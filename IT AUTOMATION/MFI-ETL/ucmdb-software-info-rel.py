...
def ucmdb_software_info(tql_json,tql_name):
    ...
    try:
        ...
        sys.exit(1)
    #Normalize the data
    df = pd.json_normalize(data, record_path=['relations'])
    df1 = pd.DataFrame(df,columns=['end1Id','end2Id'])
    df1.to_csv(output_csv,index=False)
    print("CSV Path Location is :",output_csv)
    end = time.time()
    print("The time of execution of above program is :", end-start)
#calling function
ucmdb_software_info("C://Users//rnsri//Software-ucmdb//SEP1//Node_InstalledSoftware_EUC1.json","Node_InstalledSoftware_EUC1")